...
TArray<float> UPandemicBlueprintFunctionLibrary::SortFloatArray(TArray<float> FloatArray) {
    return TArray<float>();
}
...
bool UPandemicBlueprintFunctionLibrary::RemovePlayerFromTeam(FTeamData& TeamToEdit, const APlayerState* PlayerToRemove) {
    return false;
}
...
FVote UPandemicBlueprintFunctionLibrary::MakeVote(EVoteType InVoteType, TArray<FString> InOptions, TArray<int32> InVoteTeamIndices, float InRequiredPercentage, int32 InParticipants, bool bInEndWhenAllVoted, int32 Duration) {
    return FVote{};
}
...
FKickData UPandemicBlueprintFunctionLibrary::MakeKickData(const APlayerState* InInitiatingPlayer, const APlayerState* InPlayerToKick, int32 InTeamIndex, const FString& InKickReason, int32 InNoVoteCount, int32 InYesVoteCount, EVoteKickState InCurrentState) {
    return FKickData{};
}
...
AActor* UPandemicBlueprintFunctionLibrary::GetClosestActorByBounds(const TArray<AActor*>& Actors, FVector Location, FVector& ClosestPoint) {
    return NULL;
}
...
void UPandemicBlueprintFunctionLibrary::ApplyScoreWithDamage(APlayerState* PlayerState, const FGameplayTag& Tag, TSubclassOf<UDamageType> DamageType) {
}
...
int32 UPandemicBlueprintFunctionLibrary::AddUniquePlayerToTeam(FTeamData& TeamToEdit, const APlayerState* PlayerToAdd) {
    return 0;
}
