...
public:
    ...
    UFUNCTION(BlueprintCallable)
    static TArray<float> SortFloatArray(TArray<float> FloatArray);
    ...
    UFUNCTION(BlueprintCallable)
    static bool RemovePlayerFromTeam(UPARAM(Ref) FTeamData& TeamToEdit, const APlayerState* PlayerToRemove);
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    static FVote MakeVote(EVoteType InVoteType, TArray<FString> InOptions, TArray<int32> InVoteTeamIndices, float InRequiredPercentage, int32 InParticipants, bool bInEndWhenAllVoted, int32 Duration);
    ...
    UFUNCTION(BlueprintCallable, BlueprintPure)
    static FKickData MakeKickData(const APlayerState* InInitiatingPlayer, const APlayerState* InPlayerToKick, int32 InTeamIndex, const FString& InKickReason, int32 InNoVoteCount, int32 InYesVoteCount, EVoteKickState InCurrentState);
...
public:
    ...
    UFUNCTION(BlueprintCallable)
    static AActor* GetClosestActorByBounds(const TArray<AActor*>& Actors, FVector Location, FVector& ClosestPoint);
...
public:
    ...
    static void ApplyScoreWithDamage(APlayerState* PlayerState, const FGameplayTag& Tag, TSubclassOf<UDamageType> DamageType);
    
    UFUNCTION(BlueprintCallable)
    static bool AddVote(FVote Vote, FUniqueVote NewVote);
    
    UFUNCTION(BlueprintCallable)
    static int32 AddUniquePlayerToTeam(UPARAM(Ref) FTeamData& TeamToEdit, const APlayerState* PlayerToAdd);
